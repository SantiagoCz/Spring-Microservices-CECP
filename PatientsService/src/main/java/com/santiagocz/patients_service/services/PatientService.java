package com.santiagocz.patients_service.services;

import com.santiagocz.patients_service.domain.entities.Patient;
...
import com.santiagocz.patients_service.exceptions.PatientConflictException;
import com.santiagocz.patients_service.exceptions.PatientNotFoundException;
import com.santiagocz.patients_service.repositories.PatientRepository;
...
public class PatientService {
    ...
    public PatientResponseDto update(Long id, PatientRequestDto dto) {
        ...
        if (patient.getStatus() == Status.INACTIVE) {
            throw new PatientConflictException("No se puede modificar un paciente inactivo");
        }
    ...
    public void deactivate(Long id) {
        Patient patient = getEntityById(id);
        if (patient.getStatus() == Status.INACTIVE) {
            throw new PatientConflictException("Paciente ya inactivo");
        }
    ...
    public void activate(Long id) {
        ...
        if (patient.getStatus() == Status.ACTIVE) {
            throw new PatientConflictException("Paciente ya activo");
        }
    ...
    private void validateDniNotInUse(String dni) {
        if (patientRepository.existsByDni(dni)) {
            throw new PatientConflictException("Ya existe un paciente con el DNI: " + dni);
        }
    }
